...
import type { InitAnswers, TemplateContext } from './types.js';
import {
  buildTemplateContext,
  renderAgentUpManifest,
  renderAgentsMd,
  renderClaudeAgent,
  renderClaudeCommand,
  renderClaudeHooksReadme,
  renderClaudeMcp,
  renderClaudeOutputRule,
  renderClaudeProjectRule,
  renderClaudeRoot,
  renderClaudeSettings,
  renderClaudeSettingsLocalExample,
  renderClaudeSkill,
  renderClaudeVerificationRule,
  renderCursorAgent,
  renderCursorArchitectureDoc,
  renderCursorCommand,
  renderCursorHooksReadme,
  renderCursorMcp,
  renderCursorReadme,
  renderCursorRule,
  renderCursorSkill,
...
type FilePlanner = (filePath: string, content: string) => void;
...
function createClaudeScaffold(
  ...
  addFile(
    path.join(base, 'settings.local.example.json'),
    `${JSON.stringify(renderClaudeSettingsLocalExample(), null, 2)}\n`,
  );
...
export function writeAgentUpFiles(plannedFiles: PlannedFile[]): string[] {
...
export function applyOverwriteMode(
  ...
  overwriteMode: InitAnswers['overwriteMode'],
): PlannedFile[] {
...
export function generateAgentUpFiles(answers: InitAnswers, overrides: FileOverrides = {}): string[] {
