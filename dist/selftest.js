...
export function runSelfTests() {
    ...
    assert('mcpServers' in claudeMcp &&
        typeof claudeMcp.mcpServers === 'object' &&
        claudeMcp.mcpServers !== null &&
        Object.keys(claudeMcp.mcpServers).length === 0, 'renderClaudeMcp should not reference missing example scripts');
    const cursorMcp = renderCursorMcp();
    assert('mcpServers' in cursorMcp &&
        typeof cursorMcp.mcpServers === 'object' &&
        cursorMcp.mcpServers !== null &&
        Object.keys(cursorMcp.mcpServers).length === 0, 'renderCursorMcp should not reference missing example scripts');
    ...
    try {
        ...
        fs.writeFileSync(path.join(phpRoot, 'composer.json'), JSON.stringify({ name: 'acme/demo-app', description: 'Composer app' }, null, 2), 'utf8');
