...
export function emptyProjectInfo(name) {
    return {
        name,
        description: '',
        dockerEnabled: false,
...
export function detectProjectInfo(projectRoot) {
    ...
    if (fs.existsSync(composerJsonPath)) {
        const composer = safeReadJson(composerJsonPath);
        info.repoType = 'php';
        info.language = 'php';
    ...
    if (fs.existsSync(pomXmlPath) || fs.existsSync(buildGradlePath)) {
        ...
        const javaText = [
            ...
            .join('\n')
            .toLowerCase();
        if (javaText.includes('spring-boot'))
            info.framework = 'Spring Boot';
        else if (javaText.includes('quarkus'))
            info.framework = 'Quarkus';
        else if (javaText.includes('micronaut'))
            info.framework = 'Micronaut';
        else
            info.framework = 'Plain Java';
...
function detectDockerAndDatabase(projectRoot, info) {
...
function detectGoFramework(goModPath) {
    const text = safeReadText(goModPath).toLowerCase();
    if (text.includes('github.com/gin-gonic/gin'))
        return 'Gin';
    if (text.includes('github.com/gofiber/fiber'))
        return 'Fiber';
    if (text.includes('github.com/labstack/echo'))
        return 'Echo';
    return undefined;
}
function detectRustFramework(cargoTomlPath) {
    const text = safeReadText(cargoTomlPath).toLowerCase();
    if (text.includes('axum'))
        return 'Axum';
...
export function detectPackageManager(projectRoot, declared) {
    if (declared)
...
export function scriptCommand(packageManager, scriptName, scriptValue) {
    if (!scriptValue)
        return undefined;
...
export function findFirstExisting(root, names) {
...
export function inferDescriptionFromReadme(readmePath) {
...
export function hasComposerDependency(pkg, dep) {
